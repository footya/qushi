...
;(function(host,global) {
  global.WeiBo = global.WeiBo || {};
  var backupQ = global.WB;
  global.WB = global.WeiBo;
  WeiBo.noConflict = function(){
    global.WB = backupQ;
  };
  seajs.config({base:'./'});
  //seajs.config({base:'http://107.0.0.1/'});
  WeiBo.ready = function(callback){
    var lib = '/js/lib/lib';
    var jq =  "/js/lib/jquery/jquery-1.8.2.min";
    seajs.use([lib,jq],callback || function(){});
  };
  WeiBo.load = function(jobName,callback){
    var projectName = WeiBo.projectName || '';
     if(!projectName){
       throw new Error('未指定projectName');
     }
    var verpath = '/js/'+projectName+"/ver";
    //
    WeiBo.ready(function(){
        
        seajs.use(verpath,function(ver){
            ver.loadJob(jobName,callback);
        });
    });
  };
